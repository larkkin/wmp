...
module Robots {
    import DiagramMenuController = Common.DiagramMenuController;
    import GesturesController = Common.GesturesController;
    import DiagramEditorController = EditorCore.DiagramEditorController;
    import DiagramScene = EditorCore.DiagramScene;
    import ElementTypes = EditorCore.ElementTypes;
    import PropertyEditorController = EditorCore.PropertyEditorController;
    import NodeType = EditorCore.NodeType;
    export class RobotsDiagramEditorController extends DiagramEditorController {
        ...
        constructor($scope, $attrs) {
            ...
            document.addEventListener('mousedown', (event) => {
                this.gesturesController.onMouseDown(event)
            });
            document.addEventListener('mouseup', (event) => {
                this.gesturesController.onMouseUp(event)
            });
            $("#" + scene.getId()).mousemove((event) => {
                this.gesturesController.onMouseMove(event)
            });
            ...
            $scope.createNewDiagram = () => {
                this.menuController.createNewDiagram();
            };
            $scope.openFolderWindow = () => {
                this.menuController.openFolderWindow();
            };
            $scope.saveCurrentDiagram = () => {
                this.menuController.saveCurrentDiagram();
            };
            $scope.saveDiagramAs = () => {
                this.menuController.saveDiagramAs();
            };
            $scope.clearAll = () => {
                this.clearAll();
            };
        ...
        public handleLoadedTypes(elementTypes: ElementTypes): void {
            ...
            var categories: Map<String, Map<String, NodeType>> = elementTypes.paletteTypes.categories;
        ...
        private blankPoinerdownListener(event, x, y): void {
            if (event.button == Common.MouseButton.right) {
    ...
    angular.module("Robots").controller("RobotsDiagramEditorController", RobotsDiagramEditorController);
